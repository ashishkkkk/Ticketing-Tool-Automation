...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@type='password']"))).sendKeys("Password@1108");
                    ...
                    if (extractedPORs != null && !extractedPORs.isEmpty()) {
                        ...
                        for (String por : extractedPORs) {
                            ...
                            if (extractedValue != null && !extractedValue.isEmpty()) {
                                ...
                                if (extractedValue.toLowerCase().contains("storage location")) {
                                    storageLocationError = true;
                                }
    ...
    private static void handleStorageLocationError(WebDriver driver, WebDriverWait wait, String noteText) throws InterruptedException {
        ...
        // Part 1: Add Note
        logger.info("Adding a note to the ticket.");
        WebElement noteIcon = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("((//*[@sdtooltip=\"BOTTOMFAST\"]))[11]")));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", noteIcon);
        Thread.sleep(500);
        wait.until(ExpectedConditions.elementToBeClickable(noteIcon)).click();
        Thread.sleep(3000);
        ...
        // Click Group dropdown arrow
        WebElement groupDropdown = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("((//*[@class=\"select2-arrow\"]))[3]")));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", groupDropdown);
        Thread.sleep(500);
        wait.until(ExpectedConditions.elementToBeClickable(groupDropdown)).click();
        Thread.sleep(1000);
        
        // Scroll and click the correct Group
        clickDropdownOptionWithScroll(driver, wait, 83);
        Thread.sleep(1000); // Wait for technician list to potentially update

        // Click Technician dropdown arrow
        WebElement techDropdown = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("((//*[@class=\"select2-arrow\"]))[4]")));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", techDropdown);
        Thread.sleep(500);
        wait.until(ExpectedConditions.elementToBeClickable(techDropdown)).click();
        Thread.sleep(1000);
        
        // Scroll and click the correct Technician
        clickDropdownOptionWithScroll(driver, wait, 5);
    ...
    @SuppressWarnings({"java:S106", "java:S2629","java:S2142","java:S125"})
        ...
        for (int i : new int[]{1, 13, 5}) {
            ...
            clickDropdownOptionWithScroll(driver, wait, i);
        }
    ...
    @SuppressWarnings({"java:S106", "java:S2629","java:S2142", "java:S3776"})
        ...
        public static Map<String, String> performSAPProcessVBS(String user, String pass, List<String> porNumbers) {
            ...
            try {
                ...
                vbsContent.append("WScript.Sleep 500\n"); // Brief pause to see the window minimize
    ...
    @SuppressWarnings({"java:S106", "java:S2629","java:S2142","java:S3457","java:S135","java:S1141","java:S7467"})
        private static void clickDropdownOptionWithScroll(WebDriver driver, WebDriverWait wait, int index) {
            ...
            try {
                // For long dropdowns that load more options on scroll, we need to scroll down
                // to make the target element appear. This loop scrolls to intermediate points.
                if (index > 10) { // Only do incremental scrolling for items further down the list.
                    int[] scrollPoints = {8, 18, 28, 38, 48, 58, 68, 78, 88, 98}; // A set of points to scroll to.
                    for (int point : scrollPoints) {
                        if (point < index) {
                            try {
                                WebElement intermediateElement = driver.findElement(By.xpath("((//*[@class=\"select2-result-label\"]))[" + point + "]"));
                                ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", intermediateElement);
                                Thread.sleep(300); // Brief pause for new content to load.
                            } catch (Exception e) {
                                // If an intermediate point isn't found, the list is probably shorter than we think.
                                // We can break and try to find the final element directly.
                                logger.info("Intermediate scroll point " + point + " not found. Continuing to target.");
                                break;
                            }
                        } else {
                            // We've scrolled far enough.
                            break;
                        }
                    }
                }
    
                // Now, wait for the final target element to be present.
                WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(xpath)));
                
                // Scroll the final element into view for robustness before clicking.
                ...
                logger.log(Level.WARNING, "Failed to scroll and click dropdown option at index " + index, e);
            }
        }
